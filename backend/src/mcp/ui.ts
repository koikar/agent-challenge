export function createMainUI(brandName?: string, brandLogo?: string): string {
  const title = brandName ? `${brandName} MCP` : 'Tedix MCP'
  const logoSection = brandName && brandLogo ? `
    <div class="flex items-center gap-3">
      <img src="${brandLogo}" alt="${brandName}" class="h-8 w-auto" onerror="this.style.display='none'" />
      <h1 class="text-2xl font-semibold">${brandName} MCP</h1>
    </div>
  ` : `<h1 class="text-2xl font-semibold">🎯 Tedix MCP</h1>`
...
<body class="bg-slate-50 text-slate-900">
  <div class="mx-auto max-w-5xl p-4 md:p-8">
    <header class="mb-6 flex items-center justify-between">
      ...
      <div class="flex items-center gap-4">
        ${brandName ? `
        <button id="brand-selector-btn" class="rounded-xl border border-slate-300 px-3 py-1.5 text-slate-700 hover:bg-slate-100">
          Switch Brand
        </button>
        ` : ''}
  ...
  <script>
    ...
    const brandSelectorBtn = $("brand-selector-btn");
    if (brandSelectorBtn) {
      brandSelectorBtn.addEventListener("click", () => {
        sendJSON({ type: "show_brand_selector" });
      });
    }

    // Listen for brand selection messages
    window.addEventListener("message", (event) => {
      if (event.data.type === "brand_selected") {
        sendJSON({ 
          type: "switch_brand", 
          brandSlug: event.data.payload.brandSlug 
        });
      }
    });
  </script>
